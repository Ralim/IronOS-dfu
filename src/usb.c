...
enum usbd_request_return_codes _usbd_standard_request_endpoint(void);
enum usbd_request_return_codes _usbd_standard_request_interface(void);
enum usbd_request_return_codes _usbd_standard_request_device(void);
...
static void st_usbfs_copy_to_pm(volatile void *vPM, const void *buf,
                                uint16_t len) {
...
static void st_usbfs_copy_from_pm(void *buf, const volatile void *vPM,
                                  uint16_t len) {
  uint16_t *lbuf = buf;
...
static uint16_t _usbd_ep_write_packet(uint8_t addr, const void *buf,
                                      uint16_t len) {
  addr &= 0x7F;
...
static inline void _stall_transaction(void) {
  _ep_stall_set(0, 1);
  usb_fsm_state = IDLE;
}
...
static void usb_control_send_chunk(void) {
  if (dev_desc.bMaxPacketSize0 < datasize) {
    ...
    _usbd_ep_write_packet(0, &usbd_control_buffer[dataoff],
...
static int usb_control_recv_chunk(void) {
  uint16_t packetsize =
      MIN(dev_desc.bMaxPacketSize0, usb_req.wLength - datasize);
  uint16_t size =
      _usbd_ep_read_packet(0, &usbd_control_buffer[datasize], packetsize);
...
static enum usbd_request_return_codes usb_standard_get_descriptor(void) {
  int array_idx, descr_idx, descr_type;
  struct usb_string_descriptor *sd =
      (struct usb_string_descriptor *)usbd_control_buffer;
  ...
  case USB_DT_STRING:
    ...
    if (descr_idx == 0) {
      ...
      datasize = sd->bLength = sizeof(sd->bLength) +
      ...
      if (array_idx >= (int)(sizeof(_usb_strings) / sizeof(_usb_strings[0]))) {
      ...
      datasize = sd->bLength =
          numchars * 2 + sizeof(sd->bLength) + sizeof(sd->bDescriptorType);
...
static enum usbd_request_return_codes usb_control_request_dispatch(void) {
  // Filter out
...
static void _usb_control_setup_read(void) {
  ...
  if (usb_control_request_dispatch()) {
    ...
    if (maxdataout) {
      usb_needs_zlp =
          _needs_zlp(datasize, maxdataout, dev_desc.bMaxPacketSize0);
...
static void _usb_control_setup_write(void) {
...
static void _usbd_control_setup(void) {
  usb_complete_cb = 0;
...
void _usbd_control_in(void) {
  ...
  case STATUS_IN:
    ...
    if ((usb_req.bmRequestType == 0) &&
        (usb_req.bRequest == USB_REQ_SET_ADDRESS)) {
