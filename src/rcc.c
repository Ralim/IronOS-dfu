...
int rcc_css_int_flag(void) { return ((RCC_CIR & RCC_CIR_CSSF) != 0); }
...
void rcc_set_pll2_multiplication_factor(uint32_t mul) {
  RCC_CFGR2 =
      (RCC_CFGR2 & ~RCC_CFGR2_PLL2MUL) | (mul << RCC_CFGR2_PLL2MUL_SHIFT);
}
...
void rcc_set_pll3_multiplication_factor(uint32_t mul) {
  RCC_CFGR2 =
      (RCC_CFGR2 & ~RCC_CFGR2_PLL3MUL) | (mul << RCC_CFGR2_PLL3MUL_SHIFT);
}
...
uint32_t rcc_rtc_clock_enabled_flag(void) { return RCC_BDCR & RCC_BDCR_RTCEN; }
...
void rcc_set_prediv1(uint32_t prediv) {
  RCC_CFGR2 =
      (RCC_CFGR2 & ~RCC_CFGR2_PREDIV1) | (prediv << RCC_CFGR2_PREDIV1_SHIFT);
}

void rcc_set_prediv2(uint32_t prediv) {
  RCC_CFGR2 =
      (RCC_CFGR2 & ~RCC_CFGR2_PREDIV2) | (prediv << RCC_CFGR2_PREDIV2_SHIFT);
}
...
void rcc_clock_setup_in_hsi_out_48mhz(void) {
   ...
   * 2WS from 48-72MHz
   */
  FLASH_ACR = (FLASH_ACR & ~FLASH_ACR_LATENCY) | FLASH_ACR_LATENCY_1WS;
  /*
